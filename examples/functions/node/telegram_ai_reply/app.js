...
function toolConfig(env, query) {
  ...
  const allowedFns = String(
    ...
    "telegram_ai_digest,request_inspector,cron_tick"
  )
    .split(",")
    .map((v) => String(v).trim())
...
function inferAutoTools(text, cfg) {
  ...
  if (has("noticias") || has("news") || has("digest") || has("resumen diario")) {
    addFn("telegram_ai_digest", "?preview=true&include_ai=false&include_news=true&include_weather=false");
  }

  // Internal request inspection helper
  if (has("debug") || has("headers") || has("request")) {
    addFn("request_inspector", "?key=auto");
  }
...
exports.handler = async (event) => {
  ...
  // POST /fn/telegram_ai_reply?mode=loop&chat_id=123&prompt=Hola
  ...
  //   POST /fn/telegram_ai_reply?chat_id=123&text=Hola
  let t = null;
  if (update) {
