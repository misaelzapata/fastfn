...
def handler(event):
    ...
    bot_token = _choose_config(env.get("TELEGRAM_BOT_TOKEN"), os.getenv("TELEGRAM_BOT_TOKEN"))
    openai_key = _choose_config(env.get("OPENAI_API_KEY"), os.getenv("OPENAI_API_KEY"))
    model = env.get("OPENAI_MODEL") or os.getenv("OPENAI_MODEL") or "gpt-4o-mini"
...
def _is_unset_config(value):
    """Treat placeholder example values as unconfigured."""
    if value is None:
        return True
    text = str(value).strip()
    if not text:
        return True
    return text.lower() in {"<set-me>", "set-me", "changeme", "<changeme>", "replace-me"}


def _choose_config(local_value, fallback_value):
    """Prefer function env, but allow process env fallback for local dev."""
    if not _is_unset_config(local_value):
        return str(local_value).strip()
    if not _is_unset_config(fallback_value):
