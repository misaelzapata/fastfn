...
The demo persists its sample state under a private `.state/` folder inside this example, so it works in both Docker and native mode without depending on a shared global `/tmp` path.
