#!/usr/bin/env python3
"""Tests for rust-daemon.py runtime."""
import io
import json
import os
import shutil
...
rust_daemon = load_module(RUST_DAEMON_PATH)


def test_rust_sanitize_worker_env() -> None:
    env = {
        "PATH": "/usr/bin",
        "HOME": "/home/user",
        "LANG": "en_US.UTF-8",
    ...
    sanitized = rust_daemon._sanitize_worker_env(env)
    ...
    assert rust_daemon._worker_env_key_allowed("") is False
    assert rust_daemon._worker_env_key_allowed(None) is False
    assert rust_daemon._worker_env_key_allowed("lc_all") is True
...
def test_read_function_env_and_resolve_path() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (root / "pkg").mkdir()
        (root / "pkg" / "handler.rs").write_text("pub fn handler(_e: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }", encoding="utf-8")
        (root / "rust").mkdir()
        (root / "rust" / "rt").mkdir()
        ...
        try:
            ...
            assert rust_daemon._resolve_handler_path("pkg", None) == root / "pkg" / "handler.rs"
            ...
            root_handler = root / "handler.rs"
            root_handler.write_text(
                "pub fn handler(_e: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }",
                encoding="utf-8",
            )
            nested_dir = root / "apps" / "demo"
            nested_dir.mkdir(parents=True)
            nested_handler = nested_dir / "handler.rs"
            nested_handler.write_text(
                "pub fn handler(_e: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }",
                encoding="utf-8",
            )
            (nested_dir / "src").mkdir()
            (nested_dir / "src" / "lib.rs").write_text(
                "pub fn handler(_e: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }",
                encoding="utf-8",
            )
            nested_version = nested_dir / "v3"
            nested_version.mkdir()
            (nested_version / "handler.rs").write_text(
                "pub fn handler(_e: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }",
                encoding="utf-8",
            )
            assert rust_daemon._resolve_handler_path("public-root", None, ".") == root_handler
            assert rust_daemon._resolve_handler_path("public-demo", "v3", "apps/demo") == nested_version / "handler.rs"
            (nested_dir / "fn.config.json").write_text(json.dumps({"entrypoint": "src/lib.rs"}), encoding="utf-8")
            assert rust_daemon._resolve_handler_path("public-demo", None, "apps/demo") == nested_dir / "src" / "lib.rs"
            ...
            try:
                rust_daemon._resolve_handler_path("public-demo", None, "../escape")
            except ValueError:
                pass
            else:
            ...
            try:
                rust_daemon._resolve_handler_path("public-demo", None, 123)
            except ValueError:
                pass
            else:
            ...
            try:
                rust_daemon._resolve_handler_path("public-demo", None, "   ")
            except ValueError:
                pass
            else:
            ...
            try:
                rust_daemon._resolve_handler_path("public-demo", None, "apps/demo/../../escape")
            except ValueError:
                pass
            else:
                raise AssertionError("expected invalid escaped function source dir")

            outside_root = Path(tempfile.mkdtemp(prefix="fastfn-rust-outside-"))
            try:
                (root / "linked-out").symlink_to(outside_root, target_is_directory=True)
                try:
                    rust_daemon._resolve_handler_path("public-demo", None, "linked-out")
                except ValueError:
                    pass
                else:
            ...
            outside_fn_root = Path(tempfile.mkdtemp(prefix="fastfn-rust-linked-fn-"))
            try:
                (outside_fn_root / "handler.rs").write_text(
                    "pub fn handler(_e: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }",
                    encoding="utf-8",
                )
                (root / "linked-fn").symlink_to(outside_fn_root, target_is_directory=True)
                try:
                    rust_daemon._resolve_handler_path("linked-fn", None)
            ...
            try:
                rust_daemon._resolve_handler_path("public-demo", None, "missing/demo")
            except FileNotFoundError:
                pass
            else:
...
def test_resolve_existing_path_and_handler_path_error_edges() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        file_path = root / "file.rs"
        file_path.write_text("// ok\n", encoding="utf-8")

        assert rust_daemon._resolve_existing_path_within_root(root, file_path, want_dir=True) is None
        ...
        try:
            rust_daemon.FUNCTIONS_DIR = root
            rust_daemon.RUNTIME_FUNCTIONS_DIR = root / "rust"
            rust_daemon.RUNTIME_FUNCTIONS_DIR.mkdir()

            empty_fn = root / "empty-fn"
            empty_fn.mkdir()

            try:
                rust_daemon._resolve_handler_path("missing-fn", "v1")
                raise AssertionError("expected versioned unknown function error")
            except FileNotFoundError as exc:
                assert "unknown function" in str(exc).lower()

            try:
                rust_daemon._resolve_handler_path("empty-fn", None)
...
def test_collect_rust_support_files_respects_private_and_mixed_modes() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)

        pure_dir = root / "rust"
        pure_dir.mkdir()
        pure_handler = pure_dir / "get.health.rs"
        pure_handler.write_text("pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }\n", encoding="utf-8")
        (pure_dir / "_shared.rs").write_text("pub fn helper() -> &'static str { \"ok\" }\n", encoding="utf-8")
        (pure_dir / "get.version.rs").write_text("pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }\n", encoding="utf-8")
        (pure_dir / "shared.rs").write_text("pub fn helper2() -> &'static str { \"nope\" }\n", encoding="utf-8")

        mixed_root = root / "shop"
        mixed_root.mkdir()
        (mixed_root / "handler.rs").write_text("pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }\n", encoding="utf-8")
        mixed_admin = mixed_root / "admin"
        mixed_admin.mkdir()
        mixed_handler = mixed_admin / "get.health.rs"
        mixed_handler.write_text("pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }\n", encoding="utf-8")
        (mixed_admin / "shared.rs").write_text("pub fn helper() -> &'static str { \"ok\" }\n", encoding="utf-8")
        (mixed_admin / "get.metrics.rs").write_text("pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }\n", encoding="utf-8")

        old_functions = rust_daemon.FUNCTIONS_DIR
        old_runtime = rust_daemon.RUNTIME_FUNCTIONS_DIR
        try:
            rust_daemon.FUNCTIONS_DIR = root
            rust_daemon.RUNTIME_FUNCTIONS_DIR = root / "rust_runtime"

            pure_support = [path.name for path in rust_daemon._collect_rust_support_files(pure_handler)]
            assert pure_support == ["_shared.rs"], pure_support

            mixed_support = [path.name for path in rust_daemon._collect_rust_support_files(mixed_handler)]
            assert mixed_support == ["shared.rs"], mixed_support
        finally:
            rust_daemon.FUNCTIONS_DIR = old_functions
            rust_daemon.RUNTIME_FUNCTIONS_DIR = old_runtime


def test_rust_route_detection_helpers_and_entrypoint_validation() -> None:
    assert rust_daemon._is_explicit_file_route("") is False
    assert rust_daemon._is_explicit_file_route("...") is False
    assert rust_daemon._is_explicit_file_route("get.health") is True
    assert rust_daemon._is_explicit_file_route("users.[id]") is True

    assert rust_daemon._safe_entrypoint_value(None) is None
    assert rust_daemon._safe_entrypoint_value("  src\\lib.rs  ") == "src/lib.rs"
    assert rust_daemon._safe_entrypoint_value("/abs.rs") is None
    assert rust_daemon._safe_entrypoint_value("../bad.rs") is None
    assert rust_daemon._safe_entrypoint_value("nested/../bad.rs") is None

    with tempfile.TemporaryDirectory() as tmp:
        fn_dir = Path(tmp)
        cfg_path = fn_dir / "fn.config.json"

        assert rust_daemon._has_valid_config_entrypoint(fn_dir) is False
        ...
        cfg_path.write_text(json.dumps({"entrypoint": "missing.rs"}), encoding="utf-8")
        assert rust_daemon._has_valid_config_entrypoint(fn_dir) is False

        cfg_path.write_text(json.dumps({"entrypoint": "../bad.rs"}), encoding="utf-8")
        assert rust_daemon._has_valid_config_entrypoint(fn_dir) is False

        (fn_dir / "custom.rs").write_text(
            "pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }\n",
            encoding="utf-8",
        )
        cfg_path.write_text(json.dumps({"entrypoint": "custom.rs"}), encoding="utf-8")
        assert rust_daemon._has_valid_config_entrypoint(fn_dir) is True
        assert rust_daemon._dir_has_single_entry_root(fn_dir) is True

        outside_dir = Path(tempfile.mkdtemp(prefix="fastfn-rust-entry-outside-"))
        try:
            outside_file = outside_dir / "escape.rs"
            outside_file.write_text(
                "pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({}) }\n",
                encoding="utf-8",
            )
            link_path = fn_dir / "linked.rs"
            try:
                link_path.symlink_to(outside_file)
            except (OSError, NotImplementedError):
                pass
            else:
                cfg_path.write_text(json.dumps({"entrypoint": "linked.rs"}), encoding="utf-8")
                assert rust_daemon._has_valid_config_entrypoint(fn_dir) is False
        finally:
            shutil.rmtree(outside_dir, ignore_errors=True)

        cfg_path.unlink()
        assert rust_daemon._dir_has_single_entry_root(fn_dir) is False
...
def test_ensure_rust_binary_copies_private_support_files() -> None:
    class Proc:
        def __init__(self, returncode=0, stdout="", stderr=""):
            self.returncode = returncode
            self.stdout = stdout
    ...
    with tempfile.TemporaryDirectory() as tmp:
        handler = Path(tmp) / "get.health.rs"
        handler.write_text(
            "mod _shared;\nuse serde_json::json;\n\npub fn handler(_event: serde_json::Value) -> serde_json::Value { json!({\"helper\": _shared::value()}) }\n",
            encoding="utf-8",
        )
        (Path(tmp) / "_shared.rs").write_text("pub fn value() -> &'static str { \"ok\" }\n", encoding="utf-8")
        (Path(tmp) / "get.version.rs").write_text(
            "pub fn handler(_event: serde_json::Value) -> serde_json::Value { serde_json::json!({\"ok\":true}) }\n",
            encoding="utf-8",
        )
        build_src = Path(tmp) / ".rust-build" / "src"
        build_src.mkdir(parents=True)
        stale_rs = build_src / "stale.rs"
        stale_rs.write_text("pub fn stale() {}\n", encoding="utf-8")
        ...
        try:
            ...
            def fake_run(cmd, capture_output, text, cwd, timeout, check):  # noqa: ARG001
                ...
                assert (src_dir / "_shared.rs").is_file()
                assert not (src_dir / "get.version.rs").exists()
                assert not stale_rs.exists()
                out = build_dir / "target" / "release"
                out.mkdir(parents=True, exist_ok=True)
                bin_path = out / "fn_handler"
                bin_path.write_bytes(b"\x7fELF" + b"\x00" * 100)
                bin_path.chmod(0o755)
            ...
            rust_daemon.subprocess.run = fake_run
            binary = rust_daemon._ensure_rust_binary(handler)
...
def test_additional_edge_branches() -> None:
    ...
    try:
        ...
        assert sub_env_after.get("UNIT_PREV") is None
    finally:
        if old_prev is None:
