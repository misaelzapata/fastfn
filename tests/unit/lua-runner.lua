...
local function test_assistant_modes_and_providers()
  ...
  local http_stub = {
    request = function(opts)
      ...
      if url:find("/responses", 1, true) then
        if http_mode == "openai_transport_err" then
          return nil, "openai_transport_down"
        end
      ...
      if url:find("/v1/messages", 1, true) then
        if http_mode == "claude_transport_err" then
          return nil, "claude_transport_down"
        end
  ...
  with_module_stubs({
    ...
    local resolve_mode = get_upvalue(assistant.generate, "resolve_mode")
    local normalize_mode = get_upvalue(resolve_mode, "normalize_mode")
    local prompt_looks_like_chat = get_upvalue(resolve_mode, "prompt_looks_like_chat")
    local openai_chat = get_upvalue(assistant.generate, "openai_chat")
    local openai_generate = get_upvalue(assistant.generate, "openai_generate")
    local openai_request_text = get_upvalue(openai_generate, "openai_request_text")
    local extract_output_text = get_upvalue(openai_request_text, "extract_output_text")
    local format_chat_history = get_upvalue(openai_chat, "format_chat_history")
    local format_test_result = get_upvalue(openai_chat, "format_test_result")
    local claude_generate = get_upvalue(assistant.generate, "claude_generate")
    local claude_request_text = get_upvalue(claude_generate, "claude_request_text")
    local extract_anthropic_text = get_upvalue(claude_request_text, "extract_anthropic_text")
    local assistant_enabled = get_upvalue(assistant.generate, "assistant_enabled")
    local assistant_env_bool = get_upvalue(assistant_enabled, "env_bool")
    assert_true(type(normalize_mode) == "function", "assistant normalize_mode helper")
    assert_true(type(prompt_looks_like_chat) == "function", "assistant prompt_looks_like_chat helper")
    assert_true(type(extract_output_text) == "function", "assistant extract_output_text helper")
    assert_true(type(format_chat_history) == "function", "assistant format_chat_history helper")
    assert_true(type(format_test_result) == "function", "assistant format_test_result helper")
    assert_true(type(extract_anthropic_text) == "function", "assistant extract_anthropic_text helper")
    assert_true(type(assistant_env_bool) == "function", "assistant env_bool helper")

    assert_eq(normalize_mode("weird"), "generate", "assistant invalid mode fallback")
    assert_eq(prompt_looks_like_chat(""), false, "assistant empty prompt is not chat")
    assert_eq(prompt_looks_like_chat("how does this work"), true, "assistant marker prompt should be chat")
    assert_eq(prompt_looks_like_chat("build endpoint"), false, "assistant non-chat prompt fallback")
    assert_eq(extract_output_text(nil), nil, "assistant extract_output_text nil payload")
    assert_eq(extract_output_text({ output = "bad" }), nil, "assistant extract_output_text non-table output")
    assert_eq(extract_anthropic_text(nil), nil, "assistant extract_anthropic_text nil payload")
    assert_eq(extract_anthropic_text({ content = "bad" }), nil, "assistant extract_anthropic_text non-table content")
    assert_true(type(format_test_result({ status = 201, latency_ms = 5, route = "/ok", ok = true })) == "string", "assistant format_test_result success summary")
    local long_history = {}
    for i = 1, 14 do
      long_history[#long_history + 1] = { role = "user", text = string.rep("x", 450) .. tostring(i) }
    end
    local formatted_history = format_chat_history(long_history)
    assert_true(type(formatted_history) == "string" and formatted_history:find("...", 1, true) ~= nil, "assistant format_chat_history truncates long entries")
    with_env({ FN_ASSISTANT_ENABLED = false }, function()
      assert_eq(assistant_env_bool("FN_ASSISTANT_ENABLED", true), true, "assistant env_bool nil uses default")
    end)
    ...
    with_env({
      FN_ASSISTANT_ENABLED = "maybe",
      FN_ASSISTANT_PROVIDER = "mock",
    }, function()
      local text, err = assistant.generate({ prompt = "x" })
      assert_eq(text, nil, "assistant invalid enabled flag should fallback to disabled")
      assert_eq(err, "assistant disabled", "assistant invalid enabled flag error")
    end)
    ...
    with_env({
      ...
      local text_py, err_py, mode_py = assistant.generate({
        mode = "invalid",
        runtime = "python",
        name = "genpy",
      })
      assert_eq(err_py, nil, "assistant mock generate python err")
      assert_eq(mode_py, "generate", "assistant mock invalid mode fallback to generate")
      assert_true(type(text_py) == "string" and text_py:find("def handler", 1, true) ~= nil, "assistant mock python generate")

      local text_node, err_node = assistant.generate({
        mode = "generate",
        runtime = "node",
        name = "gennode",
      })
      assert_eq(err_node, nil, "assistant mock generate node err")
      assert_true(type(text_node) == "string" and text_node:find("exports.handler", 1, true) ~= nil, "assistant mock node generate")

      local text_other, err_other = assistant.generate({
        mode = "generate",
        runtime = "rust",
        name = "genrust",
      })
      assert_eq(err_other, nil, "assistant mock generate unsupported runtime err")
      assert_true(type(text_other) == "string" and text_other:find("unsupported runtime", 1, true) ~= nil, "assistant mock unsupported runtime text")

      local text_auto_gen, err_auto_gen, mode_auto_gen = assistant.generate({
        mode = "auto",
        runtime = "python",
        name = "autogen",
        prompt = "build endpoint",
      })
      assert_eq(err_auto_gen, nil, "assistant mock auto-generate err")
      assert_eq(mode_auto_gen, "generate", "assistant auto mode should pick generate for non-chat prompt")
      assert_true(type(text_auto_gen) == "string" and text_auto_gen:find("def handler", 1, true) ~= nil, "assistant mock auto-generate text")
    end)
    ...
    with_env({
      ...
      local chat_history = {}
      for i = 1, 14 do
        chat_history[#chat_history + 1] = { role = "user", text = string.rep("m", 450) .. tostring(i) }
      end
      local text, err, mode = assistant.generate({
        mode = "chat",
        runtime = "python",
        ...
        current_code = string.rep("a", 13050),
        chat_history = chat_history,
        test_result = { ok = true, status = 200, latency_ms = 2, route = "/probe" },
      })
      assert_eq(err, nil, "assistant openai chat err")
      assert_eq(mode, "chat", "assistant openai chat mode")
      assert_true(type(text) == "string" and text:find("openai-text", 1, true) ~= nil, "assistant openai chat text")
    end)
    ...
    with_env({
      ...
      local text, err, mode = assistant.generate({
        mode = "chat",
        runtime = "python",
        name = "chat-openai-empty",
        prompt = "what does this function do?",
        current_code = "",
      })
      assert_eq(err, nil, "assistant openai empty-code chat err")
      assert_eq(mode, "chat", "assistant openai empty-code chat mode")
      assert_true(type(text) == "string" and text:find("openai-text", 1, true) ~= nil, "assistant openai empty-code chat text")
    end)
    ...
    with_env({
      ...
      http_mode = "openai_transport_err"
      local text, err = assistant.generate({ prompt = "openai transport error" })
      assert_eq(text, nil, "assistant openai transport error text")
      assert_eq(err, "openai_transport_down", "assistant openai transport error value")
    end)
    ...
    with_env({
      ...
      local text, err = assistant.generate({ prompt = "missing key" })
      assert_eq(text, nil, "assistant claude missing key text")
      assert_true(type(err) == "string" and err:find("ANTHROPIC_API_KEY not set", 1, true) ~= nil, "assistant claude missing key err")
    end)
    ...
    with_env({
      ...
      http_mode = "claude_transport_err"
      local text, err = assistant.generate({ prompt = "claude transport error" })
      assert_eq(text, nil, "assistant claude transport error text")
      assert_eq(err, "claude_transport_down", "assistant claude transport error value")
    end)
    ...
    with_env({
      ...
      http_mode = "claude_ok"
      local text0, err0, mode0 = assistant.generate({
        mode = "chat",
        prompt = "explain",
        current_code = "",
      })
      assert_eq(err0, nil, "assistant claude empty-code chat err")
      assert_eq(mode0, "chat", "assistant claude empty-code chat mode")
      assert_true(type(text0) == "string" and text0:find("claude-text", 1, true) ~= nil, "assistant claude empty-code chat text")

      local text1, err1, mode1 = assistant.generate({
        mode = "chat",
        prompt = "explain long",
        current_code = string.rep("c", 13050),
      })
      assert_eq(err1, nil, "assistant claude long-code chat err")
      assert_eq(mode1, "chat", "assistant claude long-code chat mode")
      assert_true(type(text1) == "string" and text1:find("claude-text", 1, true) ~= nil, "assistant claude long-code chat text")
    end)
    ...
    with_env({
      ...
      local status = assistant.status()
      assert_eq(status.provider, "claude", "assistant auto provider should pick claude when only anthropic key exists")
    end)
    ...
    with_env({
      ...
      local status = assistant.status()
      assert_eq(status.provider, "mock", "assistant auto provider should fallback to mock when no keys exist")
    end)
    ...
    with_env({ OPENAI_API_KEY = "test-key" }, function()
      local cjson_prev = get_upvalue(openai_request_text, "cjson")
      local swapped = set_upvalue(openai_request_text, "cjson", {
        encode = function()
          return nil
        end,
        decode = cjson_prev and cjson_prev.decode or cjson.decode,
      })
      assert_true(swapped, "assistant openai_request_text cjson swap")
      local text, err = openai_request_text({}, "sys", "user")
      assert_eq(text, nil, "assistant openai_request_text encode error text")
      assert_eq(err, "encode error", "assistant openai_request_text encode error value")
      set_upvalue(openai_request_text, "cjson", cjson_prev)
    end)

    with_env({ ANTHROPIC_API_KEY = "test-key" }, function()
      local cjson_prev = get_upvalue(claude_request_text, "cjson")
      local swapped = set_upvalue(claude_request_text, "cjson", {
        encode = function()
          return nil
        end,
        decode = cjson_prev and cjson_prev.decode or cjson.decode,
      })
      assert_true(swapped, "assistant claude_request_text cjson swap")
      local text, err = claude_request_text({}, "sys", "user")
      assert_eq(text, nil, "assistant claude_request_text encode error text")
      assert_eq(err, "encode error", "assistant claude_request_text encode error value")
      set_upvalue(claude_request_text, "cjson", cjson_prev)
    end)
  end)
end
