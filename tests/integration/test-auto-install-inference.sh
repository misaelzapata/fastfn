...
assert_dependency_resolution_error() {
  ...
  DETAIL_JSON="$payload" RUNTIME="$runtime" EXPECTED="$expected_fragment" python3 - <<'PY'
import json
import os
obj = json.loads(os.environ["DETAIL_JSON"])
meta = obj.get("metadata") or {}
dep = meta.get("dependency_resolution") or {}
assert dep.get("runtime") == os.environ["RUNTIME"], dep
assert dep.get("last_install_status") == "error", dep
err = str(dep.get("last_error") or "")
assert os.environ["EXPECTED"].lower() in err.lower(), err
PY
}

mkdir -p "$WORK_DIR/python/infer-py-fail"
cat > "$WORK_DIR/python/infer-py-fail/app.py" <<'PY'
import json
...
cat > "$WORK_DIR/node/infer-node-fail/app.js" <<'JS'
