...
wait_for_openapi_path_state() {
  ...
  for _ in $(seq 1 "$timeout"); do
    ...
    if [[ "$want" == "present" ]] && python3 "$HELPER_PY" openapi-route-present --json "$openapi_json" --route "$path" >/dev/null 2>&1
    then
      seen=1
      break
    fi
    if [[ "$want" == "absent" ]] && python3 "$HELPER_PY" openapi-route-absent --json "$openapi_json" --route "$path" >/dev/null 2>&1
    then
      seen=1
      break
    fi
