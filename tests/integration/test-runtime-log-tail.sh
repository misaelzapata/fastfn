...
assert_invoke_writes_runtime_log() {
  ...
  for _ in $(seq 1 40); do
    ...
    if [[ "$code" == "200" ]]; then
      if python3 "$HELPER_PY" logs-runtime-line \
        --file /tmp/fastfn-runtime-log-tail-runtime.json \
        --runtime python \
        --fn echo-logs \
        --version default \
        --stream stdout \
        --contains "[python]" "[fn:echo-logs@default stdout]" "'id': '42'" "'name': 'Debug'" \
        --not-contains stderr >/dev/null 2>&1
      then
        found=1
        break
      fi
  ...
  python3 "$HELPER_PY" logs-stream-empty --file /tmp/fastfn-runtime-log-tail-stderr.json --stream stderr
}
