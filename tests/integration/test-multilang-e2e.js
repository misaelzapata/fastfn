...
const BASE_URL = process.env.BASE_URL || process.env.FASTFN_TEST_BASE_URL || "http://127.0.0.1:8080";
...
async function run() {
  ...
  await checkEndpoint("/blog", "python", (body) => {
    assert.equal(body.helper, "blog/_shared.py");
  });
  ...
  if (mappedRoutes["/rust/version"]) {
    await checkEndpoint("/rust/version", "rust", (body) => {
      assert.equal(body.version, "v1");
    });
  } else {
    console.log("Skipping /rust/version (route not mapped in current runtime set)");
  }
