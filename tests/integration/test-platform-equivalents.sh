...
assert_status_and_contains GET "/auth/profile" "401" "missing_token"
...
if ! grep -qi '"role":"admin"' "$profile_file"; then
...
if ! grep -qi '"invalid_signature"' "$bad_sig_body"; then
  echo "FAIL webhook invalid signature missing invalid_signature marker"
  cat "$bad_sig_body" || true
  rm -f "$bad_sig_body"
  exit 1
fi
...
if [[ "$dup_code" != "200" ]]; then
  echo "FAIL webhook duplicate expected=200 got=$dup_code"
  cat "$dup_body" || true
  rm -f "$dup_body"
  exit 1
fi
if ! grep -qi '"duplicate":true' "$dup_body"; then
  echo "FAIL webhook duplicate missing duplicate=true"
  cat "$dup_body" || true
  rm -f "$dup_body"
  exit 1
fi
...
assert_status_and_contains GET "/api/v1/orders" "200" "\"id\":${order_id}"
assert_status_and_contains GET "/api/v1/orders/${order_id}" "200" "\"id\":${order_id}"
...
if ! grep -qi '"status":"shipped"' "$update_file"; then
