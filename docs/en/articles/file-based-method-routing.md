...
For the exact discovery edge cases, see [Zero-Config Routing](../how-to/zero-config-routing.md):
that guide covers the fixed 6-level depth limit, reserved prefixes, ASCII segment warnings,
and deterministic selection when multiple compatible entry files exist.
