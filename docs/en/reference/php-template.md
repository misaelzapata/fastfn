...
> Runtime internals: FastFN talks to `php-daemon.php`, which fans out to isolated `php-worker.php` child processes per handler path. That split is internal and keeps PHP handlers isolated from each other.
PHP runtime is implemented. Use this template for `handler.php`.
