...
| `.deps/` / `node_modules/` | read |
| Runtime stdlib paths | read |
| `/tmp` | read/write |
| `/etc/ssl`, `/etc/pki` | read (TLS certificates) |
| `/usr/share/zoneinfo` | read (timezone data) |
...
- **Python:** uses a thread-local `os.environ` proxy (`_EnvOverrideProxy`). Each request sees its own `event.env` values merged transparently.
- **Node:** uses `AsyncLocalStorage` to proxy `process.env`. Each request context has its own environment overlay.
