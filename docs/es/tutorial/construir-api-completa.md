...
Usaremos `perfil_cliente` como nombre de función.
...
```bash
curl -sS 'http://127.0.0.1:8080/_fn/function?runtime=python&name=perfil_cliente' \
  -X POST \
...
```bash
curl -sS 'http://127.0.0.1:8080/_fn/function-config?runtime=python&name=perfil_cliente' \
  -X PUT \
  -H 'Content-Type: application/json' \
  --data '{
    "timeout_ms": 1800,
...
```bash
curl -sS 'http://127.0.0.1:8080/_fn/function-env?runtime=python&name=perfil_cliente' \
  -X PUT \
  -H 'Content-Type: application/json' \
...
```bash
curl -sS 'http://127.0.0.1:8080/_fn/function-code?runtime=python&name=perfil_cliente' \
  -X PUT \
  -H 'Content-Type: application/json' \
...
```bash
curl -i -sS 'http://127.0.0.1:8080/fn/perfil_cliente?id=cli_100'
```
...
```bash
curl -i -sS -X POST 'http://127.0.0.1:8080/fn/perfil_cliente' \
  -H 'x-api-key: clave-demo-123' \
  -H 'Content-Type: application/json' \
  -d '{json malo}'
```
...
```bash
curl -sS 'http://127.0.0.1:8080/openapi.json' | rg 'perfil_cliente|"get"|"post"'
```
...
```bash
curl -sS 'http://127.0.0.1:8080/_fn/function?runtime=python&name=perfil_cliente' -X DELETE
```
