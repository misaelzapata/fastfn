...
Cuando `FN_STRICT_FS=1` (habilitado por defecto en produccion), FastFN restringe operaciones de archivo dentro del codigo del handler.

**Directorios permitidos:**

| Directorio | Acceso |
|------------|--------|
| Directorio de la funcion | lectura/escritura |
| `.deps/` / `node_modules/` | lectura |
| Paths de stdlib del runtime | lectura |
| `/tmp` | lectura/escritura |
| `/etc/ssl`, `/etc/pki` | lectura (certificados TLS) |
| `/usr/share/zoneinfo` | lectura (datos de zona horaria) |

**Operaciones bloqueadas (Python):** `open()`, `os.open()`, `Path.open()`, `os.listdir()`, `os.scandir()`, `subprocess.*`, `os.system()`, `ctypes.CDLL` — bloqueadas para paths fuera de directorios permitidos.

**Operaciones bloqueadas (Node):** `readFileSync`, `writeFileSync`, `openSync`, `readdirSync` y equivalentes async — bloqueadas para paths fuera de directorios permitidos.

**Archivos de config protegidos:** `fn.config.json`, `fn.env.json`, `fn.test_events.json` — los handlers no pueden leer estos archivos ni siquiera dentro del directorio de la funcion.

Toggle: usa `FN_STRICT_FS=0` para desactivar sandboxing (solo desarrollo).

## Aislamiento de variables de entorno

FastFN aisla variables de entorno entre requests concurrentes.

- **Python:** usa un proxy thread-local de `os.environ` (`_EnvOverrideProxy`). Cada request ve sus propios valores de `event.env` mezclados transparentemente.
- **Node:** usa `AsyncLocalStorage` para proxear `process.env`. Cada contexto de request tiene su propio overlay de entorno.

**Prefijos bloqueados:** los handlers no pueden acceder a variables de entorno con estos prefijos, incluso si estan configuradas en el host:

- `FN_ADMIN_*`
- `FN_CONSOLE_*`
- `FN_TRUSTED_*`

Estos prefijos estan reservados para uso interno de la plataforma y se eliminan antes de la ejecucion del handler.
