...
class ConsoleApp {
  ...
  async init() {
    ...
    if (route.view === 'dashboard') {
      await this.showDashboard(false);
      return;
    }
  ...
  updateViewVisibility() {
    ...
    if (this.currentView === 'dashboard') {
      ...
      if (crumb) {
        crumb.style.display = 'inline';
        const label = this.currentFn ? `${this.currentFn.runtime}/${this.currentFn.name}${this.currentFn.version ? `@${this.currentFn.version}` : ''}` : '';
        crumb.textContent = label ? `Dashboard / ${label}` : 'Dashboard';
      }
      this.setSidebarActive('navDashboard');
      return;
    }
  ...
  async showDashboard(pushUrl = true) {
    const target = this.currentFn || this.functionRows[0] || null;
    if (!target) {
      this.showFunctionList(pushUrl);
      return;
    }
    await this.openFunction(target.name, target.runtime, target.version || null, {
      pushUrl: false,
      activateTab: 'monitor',
      viewMode: 'dashboard',
    });
    if (pushUrl) history.pushState({}, '', '/console/dashboard');
  }
  ...
  async openFunction(name, runtime = null, version = null, opts = {}) {
    ...
    this.currentView = opts.viewMode === 'dashboard' ? 'dashboard' : 'functionDetail';
  ...
  reloadCurrentFunction() {
    ...
    this.openFunction(this.currentFn.name, this.currentFn.runtime, this.currentFn.version, {
      pushUrl: false,
      activateTab: this.currentTab,
      viewMode: this.currentView === 'dashboard' ? 'dashboard' : 'functionDetail',
