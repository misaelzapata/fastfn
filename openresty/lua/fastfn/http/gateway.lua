...
local function should_block_runtime(runtime, runtime_cfg)
  ...
  if up ~= true then
    local ok, err = routes_mod.check_runtime_socket(runtime_cfg.socket, runtime_cfg.timeout_ms or 250)
    routes_mod.set_runtime_health(runtime, ok, ok and "ok" or err)
    return not ok
  end
  return false
end
...
if fn_name then
  runtime, requested_version, resolve_err = routes_mod.resolve_legacy_target(fn_name, requested_version)
  if not runtime then
    if resolve_err then
      write_response(409, { ["Content-Type"] = "application/json" }, json_error(resolve_err))
      return
    end
    write_response(404, { ["Content-Type"] = "application/json" }, json_error("unknown function or version"))
    return
  end
else
  runtime, fn_name, requested_version, resolve_err = routes_mod.resolve_mapped_target(request_uri)
  if not runtime then
    if resolve_err then
      write_response(409, { ["Content-Type"] = "application/json" }, json_error(resolve_err))
      return
...
if not acquired then
  if acquire_err == "busy" then
    write_response(429, { ["Content-Type"] = "application/json" }, json_error("busy"))
    return
  end
  write_response(500, { ["Content-Type"] = "application/json" }, json_error("concurrency gate failure"))
  return
end
