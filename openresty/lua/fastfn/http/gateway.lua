...
local function resolve_request_target(request_uri, request_method, catalog)
  ...
  if runtime then
    return runtime, fn_name, requested_version, path_params, nil
  end
  if resolve_err then
    return nil, nil, nil, nil, resolve_err
  end

  if versioned_name then
    local resolved_rt, resolved_ver = routes_mod.resolve_named_target(versioned_name, versioned_version, catalog)
    if resolved_rt then
      return resolved_rt, versioned_name, resolved_ver, {}, nil
    end
  end
...
if type(assets_cfg) == "table" and assets_cfg.run_worker_first ~= true then
  if assets_http.try_serve(request_uri, req_method, assets_cfg, {
    ngx = ngx,
...
local request_catalog = nil
if type(routes_mod.discover_functions) == "function" then
  local ok_catalog, catalog = pcall(routes_mod.discover_functions, false)
...
local policy, policy_err = routes_mod.resolve_function_policy(runtime, fn_name, requested_version, request_catalog)
if not policy then
...
local fn_source_dir = type(routes_mod.resolve_function_source_dir) == "function"
  and routes_mod.resolve_function_source_dir(runtime, fn_name, request_catalog)
  or nil
...
local ok, run_err = xpcall(function()
  ...
  if routes_mod.runtime_is_in_process(runtime, runtime_cfg) then
    resp, err_code, err_msg = lua_runtime.call(runtime_request)
  else
    resp, err_code, err_msg, dispatch_meta = call_external_runtime(runtime, runtime_cfg, runtime_request, timeout_ms)
  end
