...
local function catalog_has_discovered_content()
  local discover = type(routes.discover_functions) == "function" and routes.discover_functions or nil
  if not discover then
    return false
  end
  local ok, catalog = pcall(discover, false)
  if not ok or type(catalog) ~= "table" then
    return false
  end
  if type(catalog.mapped_routes) == "table" and next(catalog.mapped_routes) ~= nil then
    return true
  end
  for _, runtime_entry in pairs(catalog.runtimes or {}) do
    if type(runtime_entry) == "table" and type(runtime_entry.functions) == "table" and next(runtime_entry.functions) ~= nil then
      return true
    end
  end
  return false
...
if assets_http.try_serve("/", ngx.req.get_method(), assets_cfg, {
  ngx = ngx,
...
if not catalog_has_discovered_content() then
  return write_response(404, {
