...
local DEFAULT_ASSETS_NOT_FOUND_HANDLING = "404"
...
local function is_safe_root_relative_path(path)
...
local function normalize_assets(source, base_dir)
  if type(source) ~= "table" or type(base_dir) ~= "string" or base_dir == "" then
    return nil
  end

  local directory = source.directory
  if type(directory) ~= "string" then
    directory = tostring(directory or "")
  end
  directory = directory:gsub("^%s+", ""):gsub("%s+$", "")
  if directory == "" or not is_safe_root_relative_path(directory) then
    return nil
  end

  local not_found_handling = source.not_found_handling
  if not_found_handling == nil then
    not_found_handling = DEFAULT_ASSETS_NOT_FOUND_HANDLING
  else
    not_found_handling = tostring(not_found_handling):gsub("^%s+", ""):gsub("%s+$", ""):lower()
    if not_found_handling == "spa" then
      not_found_handling = "single-page-application"
    end
  end
  if not_found_handling ~= DEFAULT_ASSETS_NOT_FOUND_HANDLING and not_found_handling ~= "single-page-application" then
    return nil
  end

  local abs_dir = base_dir .. "/" .. directory
  if not dir_exists(abs_dir) then
    return nil
  end
  ...
  return {
    ...
    run_worker_first = source.run_worker_first == true,
  }
end
...
local function relative_path_has_prefix(path, prefix)
  local rel = tostring(path or ""):gsub("^%./", "")
  local base = tostring(prefix or ""):gsub("^%./", "")
  if rel == "" or base == "" then
    return false
  end
  if rel == base then
  ...
  return rel:sub(1, #base + 1) == (base .. "/")
end

local function relative_path_intersects_prefix(path, prefix)
  return relative_path_has_prefix(path, prefix) or relative_path_has_prefix(prefix, path)
end
...
local function has_single_entry_file(path)
  local candidates = {
    ["handler.py"] = true,
    ["main.py"] = true,
    ["handler.js"] = true,
  ...
  for _, file_path in ipairs(list_files(path)) do
...
local function runtime_entrypoint_candidates(runtime)
  if runtime == "python" then
    return { "handler.py", "main.py" }
  end
  if runtime == "node" then
    return { "handler.js", "handler.ts", "index.js", "index.ts" }
  end
  if runtime == "php" then
    return { "handler.php", "index.php" }
  end
  if runtime == "lua" then
    return { "handler.lua", "main.lua", "index.lua" }
  end
  if runtime == "rust" then
    return { "handler.rs" }
  end
  if runtime == "go" then
    return { "handler.go", "main.go" }
  end
  return {}
end

local function detect_runtime_from_dir(path, cfg)
  if type(cfg) == "table" then
    local configured = cfg.runtime
    if type(configured) == "string" then
      configured = configured:gsub("^%s+", ""):gsub("%s+$", "")
      if KNOWN_RUNTIMES[configured] then
        return configured
      end
    end

    local entrypoint = cfg.entrypoint
    if type(entrypoint) == "string" and entrypoint ~= "" then
      local entry_runtime = detect_runtime_from_file(entrypoint)
      if entry_runtime then
        return entry_runtime
      end
    end
  end

  for _, runtime in ipairs({ "go", "lua", "node", "php", "python", "rust" }) do
    for _, candidate in ipairs(runtime_entrypoint_candidates(runtime)) do
      if file_exists(path .. "/" .. candidate) then
        return runtime
      end
    end
  end
...
local function explicit_function_name(abs_dir, cfg)
  local name = type(cfg) == "table" and cfg.name or nil
  if type(name) == "string" then
    name = name:gsub("^%s+", ""):gsub("%s+$", "")
  else
    name = ""
  end
  if name == "" or not is_safe_relative_path(name) then
    name = basename(abs_dir)
  end
  return name
end

local function resolve_root_file_target(functions_root, fn_name)
  if not is_safe_relative_path(fn_name) then
    return nil
  end
  local full = tostring(functions_root or "") .. "/" .. fn_name
...
local function resolve_root_function_dir(functions_root, fn_name, version)
  if not is_safe_relative_path(fn_name) then
    return nil
  end
  local dir = tostring(functions_root or "") .. "/" .. fn_name
...
local function resolve_catalog_function_entry(runtime, fn_name, catalog_override)
  if type(runtime) ~= "string" or runtime == "" or type(fn_name) ~= "string" or fn_name == "" then
    return nil
  end

  local catalog = catalog_override
  if type(catalog) ~= "table" then
    local ok
    ok, catalog = pcall(M.discover_functions, false)
    if not ok or type(catalog) ~= "table" then
      return nil
    end
  end

  local runtime_entry = ((catalog.runtimes or {})[runtime] or {})
  local functions = runtime_entry.functions or {}
  local fn_entry = functions[fn_name]
  if type(fn_entry) == "table" then
    return fn_entry
  end
  return nil
end

local function resolve_catalog_function_dir(functions_root, runtime, fn_name, version, catalog_override)
  local fn_entry = resolve_catalog_function_entry(runtime, fn_name, catalog_override)
  local source_dir = type(fn_entry) == "table" and fn_entry.source_dir or nil
  if type(source_dir) ~= "string" or source_dir == "" then
    return nil
  end
  return resolve_root_function_dir(functions_root, source_dir, version)
end

function M.resolve_function_source_dir(runtime, fn_name, catalog_override)
  local fn_entry = resolve_catalog_function_entry(runtime, fn_name, catalog_override)
  local source_dir = type(fn_entry) == "table" and fn_entry.source_dir or nil
  if type(source_dir) ~= "string" or source_dir == "" then
    return nil
...
local function resolve_runtime_file_target(functions_root, runtime, fn_name)
  ...
  return resolve_root_file_target(functions_root, fn_name)
end
...
local function resolve_runtime_function_dir(functions_root, runtime, fn_name, version)
  ...
  return resolve_root_function_dir(functions_root, fn_name, version)
end
...
function M.resolve_function_entrypoint(runtime, fn_name, version)
  ...
  if not dir then
    dir = resolve_catalog_function_dir(functions_root, runtime, fn_name, version)
    if not dir then
...
local function parse_method_and_tokens(base_no_ext)
  ...
  if explicit then
    for _, part in ipairs(parts) do
      local lower = string.lower(tostring(part or ""))
      if lower == "get" or lower == "post" or lower == "put" or lower == "patch" or lower == "delete" then
        ambiguous = true
        break
      end
    end
...
local function is_default_route_token(token)
  local lower = string.lower(tostring(token or ""))
  return lower == "index" or lower == "handler" or lower == "main"
end

local function is_dynamic_route_token(token)
...
local function is_explicit_file_route(base_no_ext)
  local _, parts, explicit, ambiguous = parse_method_and_tokens(base_no_ext)
  if ambiguous then
    return false
  end
  if explicit then
  ...
  for _, part in ipairs(parts) do
    if is_default_route_token(part) or is_dynamic_route_token(part) then
      return true
    end
  end
  return false
...
local function should_treat_file_as_route(base_no_ext, mixed_mode)
  if mixed_mode ~= true then
    return true
  end
  return is_explicit_file_route(base_no_ext)
end
...
local function normalize_route_token(segment)
  ...
  if lower == "index" or lower == "handler" or lower == "main" then
    return nil
  end
...
local function detect_file_based_routes_in_dir(abs_dir, rel_dir, mixed_mode, fs_rel_dir)
  local cfg = read_json_file(abs_dir .. "/fn.config.json")
  local has_single_entry_root = has_single_entry_file(abs_dir) or has_valid_config_entrypoint(abs_dir)
  local has_explicit_cfg = is_explicit_fn_config(cfg)
  if has_explicit_cfg and not has_single_entry_root and mixed_mode ~= true then
    return {}
  end

  local allow_hosts = resolve_inherited_allow_hosts(abs_dir, fs_rel_dir or rel_dir)
  ...
  for _, file_path in ipairs(list_files(abs_dir)) do
    ...
    if lower_name:sub(-5) ~= ".d.ts" then
      ...
      if runtime then
        ...
        if not should_ignore_file_base(base_no_ext) and should_treat_file_as_route(base_no_ext, mixed_mode) then
          local method, file_tokens, method_explicit, ambiguous_method_tokens = parse_method_and_tokens(base_no_ext)
          if ambiguous_method_tokens then
            ngx.log(ngx.WARN, "[routes] ignoring ambiguous multi-method filename: ", tostring(filename))
            goto continue
          end
          local suppress_default_entry_alias = false
          if has_explicit_cfg and has_single_entry_root and method_explicit ~= true and is_default_route_token(base_no_ext) then
            suppress_default_entry_alias = true
          end
          if suppress_default_entry_alias then
            goto continue
          end
...
local function local_assets_directory_for_dir(abs_dir)
  if type(abs_dir) ~= "string" or abs_dir == "" then
    return nil
  end
  local cfg = read_json_file(abs_dir .. "/fn.config.json")
  local assets = normalize_assets(type(cfg) == "table" and cfg.assets or nil, abs_dir)
...
load_runtime_config = function(force)
  local explicit_functions_root = os.getenv("FN_FUNCTIONS_ROOT")
  ...
  if not force then
    ...
    if raw then
      ...
      local root_matches = true
      if explicit_functions_root and explicit_functions_root ~= "" then
        root_matches = parsed and (
          parsed.functions_root == functions_root
          or (type(parsed.functions_root) == "string" and parsed.functions_root ~= "" and dir_exists(parsed.functions_root))
        )
      end
      if parsed and root_matches then
        return parsed
      end
    end
  end

  local root_cfg = read_json_file(functions_root .. "/fn.config.json") or {}
  ...
  local cfg = {
    ...
    assets = normalize_assets(root_cfg.assets, functions_root),
...
function M.discover_functions(force)
  ...
  local assets_dir = type(cfg.assets) == "table" and cfg.assets.directory or nil
  local zero_config_explicit_functions = {}

  local function runtime_scoped_function_dir_exists(runtime, fn_name)
    if not is_safe_relative_path(fn_name) then
      return false
    end
    return dir_exists(functions_root .. "/" .. runtime .. "/" .. fn_name)
  end

  local function remember_zero_config_explicit_function(abs_dir, fn_cfg)
    if type(fn_cfg) ~= "table" then
      return
    end
    local runtime = detect_runtime_from_dir(abs_dir, fn_cfg)
    if type(runtime) ~= "string" or runtime == "" or not ((cfg.runtimes or {})[runtime]) then
      return
    end
    local fn_name = explicit_function_name(abs_dir, fn_cfg)
    if type(fn_name) ~= "string" or fn_name == "" then
      return
    end
    if runtime_scoped_function_dir_exists(runtime, fn_name) then
      return
    end
    zero_config_explicit_functions[runtime] = zero_config_explicit_functions[runtime] or {}
    zero_config_explicit_functions[runtime][fn_name] = abs_dir
  end
  ...
  local function runtime_root_exposes_routes(abs_dir, rel_dir, depth)
    ...
    -- A child directory that already looks like a runtime-scoped function
    -- should not force the runtime root back into zero-config file discovery.
    if depth > 0 and (has_single_entry_file(abs_dir) or has_valid_config_entrypoint(abs_dir)) then
      return false
    end
    ...
    local file_routes = detect_file_based_routes_in_dir(abs_dir, rel_dir, false, rel_dir)
    if #file_routes > 0 then
      return true
    end

	    local local_assets_dir = local_assets_directory_for_dir(abs_dir)
	    for _, child_dir in ipairs(list_dirs(abs_dir)) do
	      local child_name = basename(child_dir)
	      if child_name and not should_skip_zero_config_dir(child_name)
	        and not (local_assets_dir and relative_path_intersects_prefix(child_name, local_assets_dir)) then
  ...
  local function should_skip_runtime_named_root_dir(sub_dir, sub_name)
    ...
    return is_runtime_root
	  ...
	  local function discover_zero_config_dir(abs_dir, rel_dir, depth, inherited_mixed, within_runtime_namespace)
	    if depth > 6 then
	      return
    end

    local current_is_single_entry_root = (has_single_entry_file(abs_dir) or has_valid_config_entrypoint(abs_dir))
    if within_runtime_namespace == true and current_is_single_entry_root then
      return
    end
    local current_mixed = inherited_mixed == true or current_is_single_entry_root
    ...
    local file_routes = detect_file_based_routes_in_dir(abs_dir, rel_dir, current_mixed, rel_dir)
    for _, item in ipairs(file_routes) do
      register_route(item.route, item.runtime, item.target, nil, item.methods, 1, item.allow_hosts)
		    ...
		    local local_assets_dir = normalize_assets(type(fn_cfg) == "table" and fn_cfg.assets or nil, abs_dir)
		    local local_assets_directory = type(local_assets_dir) == "table" and local_assets_dir.directory or nil
		    local has_explicit_cfg = is_explicit_fn_config(fn_cfg)
		    if has_explicit_cfg and current_is_single_entry_root then
		      remember_zero_config_explicit_function(abs_dir, fn_cfg)
		    end
		    local is_leaf = has_manifest or (has_explicit_cfg and current_is_single_entry_root == false)
        ...
        if depth == 0 and runtime_dirs[sub_name] then
          skip_runtime_root = should_skip_runtime_named_root_dir(sub_dir, sub_name)
	        ...
	        if assets_dir and relative_path_has_prefix(sub_rel, assets_dir) then
	          skip_runtime_root = true
	        end
	        if local_assets_directory and relative_path_intersects_prefix(sub_name, local_assets_directory) then
	          skip_runtime_root = true
	        end
	        if not skip_runtime_root then
	          local child_runtime_namespace = within_runtime_namespace == true or (depth == 0 and runtime_dirs[sub_name] == true)
	          discover_zero_config_dir(sub_dir, sub_rel, depth + 1, current_mixed, child_runtime_namespace)
        end
      end
    end
  end
  discover_zero_config_dir(functions_root, ".", 0, false, false)
  ...
  for _, runtime in ipairs(sorted_keys(cfg.runtimes or {})) do
    ...
    local function relative_function_dir(fn_dir)
      local prefix = functions_root .. "/"
      if type(fn_dir) ~= "string" or fn_dir == "" then
        return nil
      end
      if fn_dir == functions_root then
        return "."
      end
      if fn_dir:sub(1, #prefix) == prefix then
        return fn_dir:sub(#prefix + 1)
      end
      return nil
    end
    ...
    local function register_runtime_function(fn_dir, fn_name)
        local fn_entry = {
          has_default = has_single_entry_file(fn_dir) or has_valid_config_entrypoint(fn_dir),
          versions = {},
          policy = normalize_policy(read_json_file(fn_dir .. "/fn.config.json")),
          versions_policy = {},
          source_dir = relative_function_dir(fn_dir),
        }

        for _, ver_dir in ipairs(list_dirs(fn_dir)) do
          local ver = basename(ver_dir)
          if ver and ver:match("^[a-zA-Z0-9_.-]+$") and (has_single_entry_file(ver_dir) or has_valid_config_entrypoint(ver_dir)) then
        ...
        if fn_entry.has_default or #fn_entry.versions > 0 then
          ...
          local function register_mixed_runtime_routes(dir, rel_prefix, depth)
            if depth > 6 then
              return
            end

	            local fs_rel_prefix = runtime .. "/" .. rel_prefix
	            local file_routes = detect_file_based_routes_in_dir(dir, rel_prefix, true, fs_rel_prefix)
	            for _, item in ipairs(file_routes) do
	              local target_base = basename(item.target)
	              local target_base_no_ext = target_base and target_base:gsub("%.[^.]+$", "") or ""
	              local _, _, method_explicit = parse_method_and_tokens(target_base_no_ext)
	              local skip_root_entry_alias = rel_prefix == fn_name
	                and method_explicit == false
	                and is_default_route_token(target_base_no_ext)
	              if not skip_root_entry_alias then
	                register_route(item.route, runtime, item.target, nil, item.methods, 1, item.allow_hosts)
              end
              ::continue_runtime_root_route::
            end

	            local local_assets_dir = local_assets_directory_for_dir(dir)
	            for _, child in ipairs(list_dirs(dir)) do
	              local child_name = basename(child)
	              if child_name and not should_skip_zero_config_dir(child_name)
	                and not (local_assets_dir and relative_path_intersects_prefix(child_name, local_assets_dir)) then
	                local child_rel = rel_prefix ~= "" and (rel_prefix .. "/" .. child_name) or child_name
	                register_mixed_runtime_routes(child, child_rel, depth + 1)
	              end
            end
          end

          register_mixed_runtime_routes(fn_dir, fn_name, 0)
    ...
    local function has_versioned_runtime_children(dir)
      ...
      for _, child in ipairs(list_dirs(dir)) do
        ...
        if child_name and child_name:match("^[A-Za-z0-9_.-]+$") then
          if has_single_entry_file(child) or has_valid_config_entrypoint(child) then
    ...
    local function discover_runtime_dir(dir, prefix, depth)
      for _, sub in ipairs(list_dirs(dir)) do
        ...
        if name and name:match("^[a-zA-Z0-9_-]+$") then
          if has_single_entry_file(sub) or has_valid_config_entrypoint(sub) or has_versioned_runtime_children(sub) then
    ...
    for _, fn_name in ipairs(sorted_keys(zero_config_explicit_functions[runtime] or {})) do
      register_runtime_function(zero_config_explicit_functions[runtime][fn_name], fn_name)
    end
...
function M.resolve_mapped_target(path, method, host_ctx, catalog_override)
  ...
  local catalog = type(catalog_override) == "table" and catalog_override or M.discover_functions(false)
  local mapped_route_conflicts = type((catalog or {}).mapped_route_conflicts) == "table"
    and catalog.mapped_route_conflicts or {}

  if mapped_route_conflicts[route] then
    return nil, nil, nil, nil, "ambiguous function"
  end
  ...
  local dynamic_conflicts = sort_dynamic_routes(mapped_route_conflicts)
  for _, conflict_route in ipairs(dynamic_conflicts) do
    if conflict_route ~= route then
      local params = extract_dynamic_route_params(conflict_route, route)
      if params ~= nil then
        return nil, nil, nil, nil, "ambiguous function"
      end
    end
...
function M.resolve_function_policy(runtime, fn_name, version, catalog_override)
...
function M.resolve_named_target(fn_name, version, catalog_override)
  local catalog = type(catalog_override) == "table" and catalog_override or M.discover_functions(false)
