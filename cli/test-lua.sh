...
LUA_STEP_TIMEOUT_SECS="${LUA_STEP_TIMEOUT_SECS:-600}"
...
run_dc() {
  if command -v timeout >/dev/null 2>&1; then
    timeout "${LUA_STEP_TIMEOUT_SECS}s" "${DC[@]}" "$@"
    return
  fi
  "${DC[@]}" "$@"
}
...
if [[ "$LUA_BUILD_LOGS" == "1" ]]; then
  run_dc build openresty
else
  run_dc build openresty >/dev/null
fi
