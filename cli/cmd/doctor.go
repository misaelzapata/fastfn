...
var (
	...
	doctorDiscoveryScanFn = discovery.Scan
...
func runGeneralDoctorChecks(applyFix bool) doctorReport {
	...
	checks = append(checks, checkFunctionDiscovery())
...
func checkFunctionDiscovery() doctorCheck {
	dir := configuredFunctionsDir()
	if dir == "" {
		dir = "."
	}
	if _, err := os.Stat(dir); err != nil {
		return doctorCheck{
			...
			Status:  doctorStatusWarn,
			Message: fmt.Sprintf("Discovery skipped because functions-dir is missing: %s", dir),
		}
	}
	...
	if _, err := doctorDiscoveryScanFn(dir, logger); err != nil {
		return doctorCheck{
			ID:      "project.discovery",
			Status:  doctorStatusFail,
			Message: fmt.Sprintf("Function discovery failed: %v", err),
			Hint:    "Fix invalid project paths or configs before running fastfn dev/run",
		}
	}
	...
	if len(conflicts) > 0 {
		...
		return doctorCheck{
			ID:      "project.discovery",
			Status:  doctorStatusWarn,
			Message: fmt.Sprintf("Discovery found %d route conflict(s)", len(conflicts)),
			Hint:    "Rename one of the conflicting routes so Go discovery and Lua routing stay aligned",
			Details: map[string]string{
	...
	return doctorCheck{
		ID:      "project.discovery",
		Status:  doctorStatusOK,
		Message: "Function discovery completed without route conflicts",
	}
}
