...
var configWriteFileFn = os.WriteFile

func nativeNginxTempDir(runtimeDir string) string {
	return filepath.Join(runtimeDir, "openresty", "tmp")
}
...
func GenerateNativeConfig(runtimeDir string, hostPort string) (string, error) {
	...
	// 2c. Native OpenResty must keep pid/temp paths inside the extracted runtime
	// tree instead of the Docker-oriented /tmp/fastfn location.
	nativeTempDir := nativeNginxTempDir(runtimeDir)
	tempReplacements := map[string]string{
		"pid /tmp/fastfn/nginx.pid;":                          "pid logs/nginx.pid;",
		"client_body_temp_path /tmp/fastfn/client_body_temp;": filepath.ToSlash(filepath.Join(nativeTempDir, "client_body_temp")),
		"proxy_temp_path       /tmp/fastfn/proxy_temp;":       filepath.ToSlash(filepath.Join(nativeTempDir, "proxy_temp")),
		"fastcgi_temp_path     /tmp/fastfn/fastcgi_temp;":     filepath.ToSlash(filepath.Join(nativeTempDir, "fastcgi_temp")),
		"uwsgi_temp_path       /tmp/fastfn/uwsgi_temp;":       filepath.ToSlash(filepath.Join(nativeTempDir, "uwsgi_temp")),
		"scgi_temp_path        /tmp/fastfn/scgi_temp;":        filepath.ToSlash(filepath.Join(nativeTempDir, "scgi_temp")),
	}
	for needle, target := range tempReplacements {
		replacement := target
		if strings.HasSuffix(needle, ";") && !strings.HasSuffix(target, ";") {
			parts := strings.Fields(needle)
			if len(parts) > 0 {
				replacement = parts[0] + " " + target + ";"
			}
		}
		content = strings.ReplaceAll(content, needle, replacement)
	}

	for _, name := range []string{"client_body_temp", "proxy_temp", "fastcgi_temp", "uwsgi_temp", "scgi_temp"} {
		if err := os.MkdirAll(filepath.Join(nativeTempDir, name), 0o755); err != nil {
			return "", fmt.Errorf("failed to create native nginx temp dir %s: %w", filepath.Join(nativeTempDir, name), err)
		}
	}
	...
	if err := configWriteFileFn(newConfigPath, []byte(content), 0644); err != nil {
