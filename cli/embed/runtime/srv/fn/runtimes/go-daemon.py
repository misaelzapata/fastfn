...
# Worker subprocesses get a conservative env allowlist. User secrets should be
# injected explicitly via fn.env.json or request-scoped env, not ambient host env.
...
def _go_subprocess_env(extra: Optional[Dict[str, str]] = None) -> Dict[str, str]:
    env = dict(os.environ)
    env.pop("GOROOT", None)
    env.pop("GOPATH", None)
...
def _go_toolchain_sane(go_cmd: str) -> bool:
    if not go_cmd:
        return False
    try:
        proc = subprocess.run(
            ...
            timeout=5,
            check=False,
            env=_go_subprocess_env(),
        )
    except Exception:
        return False
    goroot = (proc.stdout or "").strip()
    if proc.returncode != 0 or goroot == "":
        return False
    return (Path(goroot) / "src" / "unsafe").exists()


def _system_go_command() -> Optional[str]:
    system_go = "/usr/local/go/bin/go"
    if Path(system_go).is_file() and os.access(system_go, os.X_OK) and _go_toolchain_sane(system_go):
        return system_go
    return None
...
def _resolve_go_command() -> str:
    ...
    system_go = _system_go_command()
    if configured:
        if "/" in configured or "\\" in configured:
            candidate = Path(configured)
            if candidate.is_file() and os.access(str(candidate), os.X_OK):
                if _go_toolchain_sane(str(candidate)):
                    return str(candidate)
                if system_go:
                    return system_go
                raise RuntimeError(f"FN_GO_BIN points to an unhealthy Go toolchain: {configured}")
            raise RuntimeError(f"FN_GO_BIN is not executable: {configured}")
        resolved = shutil.which(configured)
        if resolved:
            if _go_toolchain_sane(resolved):
                return resolved
            if system_go:
                return system_go
            raise RuntimeError(f"FN_GO_BIN resolved to an unhealthy Go toolchain: {resolved}")
        raise RuntimeError(f"FN_GO_BIN not found in PATH: {configured}")

    go_cmd = shutil.which("go")
    if go_cmd and _go_toolchain_sane(go_cmd):
        return go_cmd
    if system_go:
        return system_go
    if go_cmd is not None:
        raise RuntimeError(f"go found in PATH but toolchain is unhealthy: {go_cmd}")
...
def _normalize_route_base(name: str) -> str:
    return name.replace("\\", "/").rsplit("/", 1)[-1]


def _is_dynamic_route_token(token: str) -> bool:
    return bool(re.fullmatch(r"\[[A-Za-z0-9_-]+\]", token) or re.fullmatch(r"\[{1,2}\.\.\.[A-Za-z0-9_-]+\]", token))


def _is_explicit_file_route(base_name: str) -> bool:
    base = _normalize_route_base(base_name)
    if base == "":
        return False
    ...
    if tokens[0] in {"get", "post", "put", "patch", "delete"}:
        return True

    for token in tokens:
        if token in {"index", "handler", "main"} or _is_dynamic_route_token(token):
            return True
    return False
...
def _has_valid_config_entrypoint(fn_dir: Path) -> bool:
    return _resolve_config_entrypoint_path(fn_dir) is not None
...
def _resolve_config_entrypoint_path(fn_dir: Path) -> Optional[Path]:
    ...
    if not isinstance(cfg, dict):
        return None
    entrypoint = _safe_entrypoint_value(cfg.get("entrypoint"))
    if not entrypoint:
        return None
    return _resolve_existing_path_within_root(fn_dir, fn_dir / entrypoint, want_file=True)


def _dir_has_single_entry_root(fn_dir: Path) -> bool:
    if _has_valid_config_entrypoint(fn_dir):
        return True
    for candidate in ("handler.go", "main.go"):
        if (fn_dir / candidate).is_file():
            return True
    return False


def _inside_single_entry_tree(fn_dir: Path) -> bool:
    current = fn_dir.resolve(strict=False)
    roots = {
        FUNCTIONS_DIR.resolve(strict=False),
        RUNTIME_FUNCTIONS_DIR.resolve(strict=False),
    }
    while True:
        if _dir_has_single_entry_root(current):
            return True
...
def _collect_go_support_files(handler_path: Path) -> list[Path]:
    fn_dir = handler_path.parent
    inside_single_entry = _inside_single_entry_tree(fn_dir)
    out: list[Path] = []
    for sibling in sorted(fn_dir.glob("*.go")):
        if sibling.name == handler_path.name or not sibling.is_file():
            continue
        stem = sibling.stem
        if inside_single_entry:
            if _is_explicit_file_route(stem):
                continue
            out.append(sibling)
            continue
...
def _resolve_handler_path(name: str, version: Any, source_dir: Any = None) -> Path:
    ...
    if source_base is None and not version:
        ...
        if resolved_root_check is not None and resolved_root_check.suffix == ".go":
        ...
        if resolved_runtime_check is not None and resolved_runtime_check.suffix == ".go":
            return resolved_runtime_check

    if source_base is not None:
        base = source_base
    else:
        base = _resolve_existing_path_within_root(FUNCTIONS_DIR, FUNCTIONS_DIR / normalized_name, want_dir=True)
        if base is None:
            base = _resolve_existing_path_within_root(RUNTIME_FUNCTIONS_DIR, RUNTIME_FUNCTIONS_DIR / normalized_name, want_dir=True)
    ...
    explicit_entrypoint = _resolve_config_entrypoint_path(target_dir)
    if explicit_entrypoint is not None:
        return explicit_entrypoint

    for candidate in ("handler.go", "main.go"):
        candidate_path = _resolve_existing_path_within_root(target_dir, target_dir / candidate, want_file=True)
...
def _ensure_go_binary(handler_path: Path) -> Path:
    ...
    support_files = _collect_go_support_files(handler_path)
    deps_signature = {
    ...
    signature = {
        ...
        "support_files": {
            str(path.relative_to(fn_dir)): _file_signature(path) for path in support_files
        },
    ...
    with lock_path.open("a+", encoding="utf-8") as lock_file:
        ...
        try:
            ...
            for stale in build_dir.glob("*.go"):
                if stale.name not in {"user_handler.go", "fastfn_entry.go"}:
                    stale.unlink()

            copied_support_files: list[Path] = []
            for support_path in support_files:
                dest = build_dir / support_path.name
                dest.write_text(support_path.read_text(encoding="utf-8"), encoding="utf-8")
            ...
            cmd.extend(str(path) for path in copied_support_files)
            try:
                proc = subprocess.run(
                    cmd,
                    cwd=str(build_dir),
...
class _PersistentGoWorker:
    ...
    def __init__(self, binary: Path):  # pragma: no cover
        self.binary = binary
    ...
    def send_request(self, event: Dict[str, Any], timeout_ms: int) -> Dict[str, Any]:  # pragma: no cover
    ...
    def _read_exact(self, fd: int, size: int, timeout_s: float) -> Optional[bytes]:  # pragma: no cover
        data = bytearray()
...
def _prepare_request(req: Dict[str, Any]) -> tuple[Path, Path, Dict[str, Any], int]:
    ...
    handler_path = _resolve_handler_path(fn_name, version, req.get("fn_source_dir"))
