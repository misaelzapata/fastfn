...
def _dir_has_single_entry_root(fn_dir: Path) -> bool:
    if _has_valid_config_entrypoint(fn_dir):
        return True
    for candidate in ("handler.rs", "lib.rs"):
        if (fn_dir / candidate).is_file():
            return True
    if (fn_dir / "src" / "lib.rs").is_file():
        return True
    return False
...
def _collect_rust_support_files(handler_path: Path) -> list[Path]:
    fn_dir = handler_path.parent
    inside_single_entry = _inside_single_entry_tree(fn_dir)
    out: list[Path] = []
    for sibling in sorted(fn_dir.glob("*.rs")):
...
def _resolve_handler_path(name: str, version: Any, source_dir: Any = None) -> Path:
    ...
    candidates = [
        target_dir / "handler.rs",
        target_dir / "lib.rs",
        target_dir / "src" / "lib.rs",
    ]

    for candidate_path in candidates:
        resolved_candidate = _resolve_existing_path_within_root(target_dir, candidate_path, want_file=True)
...
def _ensure_rust_binary(handler_path: Path) -> Path:
    ...
    support_files = _collect_rust_support_files(handler_path)
    signature = {
        "source_mtime_ns": source_mtime,
        "support_files": {
            str(path.relative_to(handler_path.parent)): _file_signature(path) for path in support_files
        },
    ...
    with lock_path.open("a+", encoding="utf-8") as lock_file:
        ...
        try:
            ...
            for stale in src_dir.glob("*.rs"):
                if stale.name not in {"main.rs", "user_handler.rs"}:
                    stale.unlink()
            for support_path in support_files:
                dest = src_dir / support_path.name
...
def _emit_handler_logs(req: Dict[str, Any], resp: Dict[str, Any]) -> None:
    ...
    if isinstance(stdout_value, str) and stdout_value != "":
        for line in stdout_value.splitlines():
            ...
            _append_runtime_log("rust", log_line)
