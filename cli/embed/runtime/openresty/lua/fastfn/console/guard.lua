...
local function request_rate_key(scope)
  local ip = tostring(ngx.var.remote_addr or "unknown")
  return "console:rl:" .. tostring(scope or "api") .. ":" .. ip
end

local function enforce_rate_limit(scope)
  local store = state_store()
  if not store then
    return true
  end

  local window_s = math.floor(env_num("FN_CONSOLE_RATE_LIMIT_WINDOW_S", 60))
  if window_s <= 0 then
    return true
  end

  local method = ngx.req.get_method()
  local max_requests = env_num("FN_CONSOLE_RATE_LIMIT_MAX", 120)
  if scope == "write" or (scope == "api" and method ~= "GET" and method ~= "HEAD" and method ~= "OPTIONS") then
    max_requests = env_num("FN_CONSOLE_WRITE_RATE_LIMIT_MAX", 30)
  end
  max_requests = math.floor(max_requests)
  if max_requests <= 0 then
    return true
  end

  local key = request_rate_key(scope)
  local count, err = store:incr(key, 1, 0, window_s)
  if not count then
    if err then
      ngx.log(ngx.WARN, "[console] failed to increment rate limit key ", key, ": ", tostring(err))
    end
    return true
  end
  if count > max_requests then
    json_error(429, "too many console requests, try again later")
    return false
  end
  return true
end
...
function M.enforce_api(opts)
  ...
  if not enforce_rate_limit("api") then
    return false
  end
...
function M.enforce_ui()
  ...
  if not enforce_rate_limit("ui") then
    return false
  end
...
function M.enforce_write()
  if not enforce_rate_limit("write") then
    return false
  end
