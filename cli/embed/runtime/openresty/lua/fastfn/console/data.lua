...
local function is_symlink(path)
  if type(fs.is_symlink) ~= "function" then
    return false
  end
...
local function dir_exists(path)
  if type(fs.is_dir) ~= "function" then
    return false
  end
...
local function rm_path(path)
  if type(fs.remove_tree) ~= "function" then
    return false
  end
  local ok = fs.remove_tree(path)
  return ok == true
end

local function list_dirs(path)
  if type(fs.list_dirs) ~= "function" then
    return {}
  end
    ...
    metadata = {
      handler_file = app_path:match("([^/]+)$") or app_path,
	      response = {
	        ...
	        effective_include_debug_headers = type(policy) == "table" and policy.include_debug_headers == true,
	      },
      schedule = {
...
local function list_files_recursive(dir, max_depth)
  return fs.list_files_recursive(dir, max_depth, function(entry)
...
function M.function_files(runtime, name, version)
  ...
  if not path_is_under(dir, app_path)
    or not path_is_under(dir, conf_path)
    or not path_is_under(dir, env_path)
  then
    return nil, "invalid function path"
  end
...
function M.write_function_file(runtime, name, rel_path, content, version)
  ...
  if parent and not dir_exists(parent) then
    local ok = ensure_dir(parent)
    if not ok then
      return nil, "failed to create parent directory"
    end
  end
...
function M.delete_function_file(runtime, name, rel_path, version)
  ...
  local removed = fs.remove_tree(abs)
  if not removed then
