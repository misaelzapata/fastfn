...
if [ "$USE_BASH" = "1" ]; then
  ...
  run_stage "host path leaks" python3 "$ROOT_DIR/scripts/ci/check_host_path_leaks.py"
  ...
  run_stage "go tests (full cli)" "$ROOT_DIR/cli/test-go.sh"
  ...
  python -m pytest --version >/dev/null 2>&1 || python -m pip install --quiet pytest
  run_stage "python unit" python -m pytest "$ROOT_DIR/tests/unit/python/" -v \
    -W error::RuntimeWarning \
    -W error::pytest.PytestUnraisableExceptionWarning
  if has_cmd node; then
    run_stage "node unit" bash "$ROOT_DIR/scripts/ci/run_node_unit.sh"
