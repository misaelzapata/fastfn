...
FUNCTIONS_DIR = BASE_DIR / "functions" / "rust"
...
def _resolve_handler_path(name: str, version: Any) -> Path:
    ...
    if version is None or version == "":
        app_path = base / "app.rs"
        handler_path = base / "handler.rs"
        path = app_path if app_path.is_file() else handler_path
    else:
        if not isinstance(version, str) or not _VERSION_RE.match(version):
            raise ValueError("invalid function version")
        app_path = base / version / "app.rs"
        handler_path = base / version / "handler.rs"
        path = app_path if app_path.is_file() else handler_path
...
def _ensure_rust_binary(handler_path: Path) -> Path:
    ...
    proc = subprocess.run(cmd, capture_output=True, text=True, cwd=str(build_dir), check=False)
